...
public class Gamma : MonoBehaviour
{
   ...
   // public Transform point, way;
    ...
    void Gam()
    {
        if (Player.transform.position.x > transform.position.x && Rend.enabled == true)
            Rend.enabled = false;
    }
    ...
    void Update()
    {
        ...
        if (Mathf.Abs(Player.transform.position.x - transform.position.x) < dist|| Player.transform.position.x > transform.position.x)
        //   if(start)
        {
            ...
            ////       RendPlayer.color = new Color(255,255,255, Mathf.Lerp(255, 0, form));
            Rend.color = color;
             }
